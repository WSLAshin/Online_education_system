...
<body>
    ...
    <div class="itab">
  	<ul> 
    <li><a href="#tab1" class="selected">修改题库</a></li> 
  	</ul>
    </div> 
  	<div id="tab1" class="tabson">
  	<form id="form" action="${ctx}/onlinelibrary/updateOpById" method="post" onsubmit="return validate()" >
  	<input  type="hidden"   name="op_id" value="${options.op_id}" /><!-- 隐藏的id用于根据id修改 -->
    <ul class="forminfo">
    <li ><label>问题<b>*</b></label>
         <textarea   class="dfinput"  name="question" id ="question"  style="width:100%;height:140px;">${options.question}</textarea>
       <%--  <input name="question" id="question"  class="dfinput" style="width:100%;height:140px;" value="${options.question}"></input> --%>
    </li>
    <li >选项A<b>*</b>
    ...
    <li >选项D<b>*</b>
     ...
     <li><label>请选择正确项<b>*</b></label>
           A<input name="op_answer" type="radio"   value="A"  ${options.op_answer=='A'?'checked':''}  style="width:50px;margin-top:8px;"/>
           B<input name="op_answer" type="radio"   value="B"  ${options.op_answer=='B'?'checked':''}  style="width:50px;margin-top:8px;"/>
           C<input name="op_answer" type="radio"   value="C"  ${options.op_answer=='C'?'checked':''}  style="width:50px;margin-top:8px;"/>
           D<input name="op_answer" type="radio"   value="D"  ${options.op_answer=='D'?'checked':''} style="width:50px;margin-top:8px;"/>
    </li>
    <li style="display:inline;"><label>解析<b>*</b></label>
      <textarea   class="dfinput"  name="parse" id ="parse"  style="width:100%;height:100px;">${options.parse}</textarea>
        <%--  <input  type="text" class="dfinput"  name="parse" id ="parse"  style="width:650px;" value="${options.parse}"/> --%>
    </li>
		...
		<select  name="up_id" id="up_id" class="dfinput" >
                       <option value=" " >----请选择----</option>
						<c:forEach items="${vedioes}" var="item" >
							<option value="${item.up_id}" ${item.up_id eq options.up_id ? "selected='selected'":""} >${item.title}</option>	
						</c:forEach>
		</select>
    </li>
