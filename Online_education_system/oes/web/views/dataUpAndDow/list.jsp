...
<body>
    ...
    <div class="tools">
    	<ul class="toolbar">
	         <li ><span><a href="${ctx}/addData"> <img src="${pageContext.request.contextPath}/resources/images/t01.png" /></a></span>资料上传</li>
        </ul>
    <!--  综合查询-->  
   <div style="float:left;width:570px;height:45px;border:1px solid #F4F4F4;border-radius:10px;" >
		 <form method="POST"  id="form" action="${ctx}/queryData">
		...
		<c:forEach items="${page.result}" var="item">
				<tr id="1">
				    ...
				    <td width="60">${item.upload_time}</td>
					 <td width="60">
                       <%-- <a href="${ctx}/download?filename=${item.file_name}">
                        ${item.file_name}下载
                       </a> --%>
                       <a href="${ctx}/download?filename=${item.file_name}&data_id=${item.data_id}">
                                                                              下载
                       </a>
					  <a href='${ctx}/deleteDataById/${item.data_id}'
